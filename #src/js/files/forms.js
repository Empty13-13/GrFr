...
class Select {
  ...
  #placeholder() {
    ...
    if(placeholder) {
      this.$el.querySelector('.select__input').classList.add('_placeholder')
    }
  }
  ...
  get current() {
    this.$el.querySelector('.select__input').classList.remove('_placeholder')
...
if(!!select1) {
  let dataSelect1 = setCollection(select1)

  new Select('#select1', {
    placeholder: 'Bitte wählen Sie Ihr Thema*',
    data: dataSelect1,
  })
  ...
  function setCollection(select) {
    ...
    children.forEach((item, index) => {
      result.push({value: `${item.textContent}`, id: index})
    })
    return result
  }
}
...
// endregion
//region Placeholders inputs,textarea
let inputs = document.querySelectorAll('input');
let TAs = document.querySelectorAll('textarea');
let parendNodeActive = null
let activeIndex = 0
inputs.forEach((input, index) => {
  addPlaceholder(input)
  addFocus(input)
})
TAs.forEach((input, index) => {
  addPlaceholder(input)
  addFocus(input)
})
...
function addPlaceholder(input) {
  if (input.dataset.value) {
    let parendNode = input.parentNode
    if (parendNode !== parendNodeActive) {
      parendNodeActive = parendNode
      activeIndex = 0
    }
    let div = document.createElement('div')
    div.classList.add('inputWithPlaceholder')
    let span = document.createElement('span')
    span.textContent = input.dataset.value
    div.append(input)
    div.append(span)
    parendNode.insertBefore(div, parendNode.children[activeIndex]);
...
function addFocus(item) {
  item.addEventListener("focus", function (e) {
    let span = item.parentNode.querySelector('span')
